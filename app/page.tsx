...
import { Bot, Play, Plus, History, Settings, BarChart3, Users, GitBranch, MessageSquare, Plug } from 'lucide-react';
import { predefinedAgents } from '@/lib/agents';
import type { Agent } from '@/lib/types';

// React Flow uses browser APIs — must be dynamically imported with ssr: false
const FlowsPanel = dynamic(() => import('@/components/flows/FlowsPanel').then((m) => m.FlowsPanel), { ssr: false });

type ActiveTab = 'missions' | 'fleet' | 'history' | 'flows' | 'assistants' | 'integrations';
...
export default function TeamForge() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleLaunchMission(); }
  };
  ...
  // Panels that occupy the full content area (no fleet sidebar, no right sidebar)
  const isFullPanel = activeTab === 'flows' || activeTab === 'assistants' || activeTab === 'integrations';
    ...
    <div className="flex h-screen overflow-hidden bg-zinc-950">
      ...
      <div className="w-72 border-r border-zinc-800 bg-zinc-950 flex flex-col">
        ...
        <div className="px-3 pt-5 flex-1">
          {/* Core */}
          <div className="text-[9px] uppercase tracking-[0.12em] text-zinc-600 px-4 mb-2">Core</div>
          {[
            { id: 'missions' as ActiveTab, icon: Play, label: 'New Mission' },
            { id: 'fleet' as ActiveTab, icon: Users, label: 'Agent Fleet', badge: agents.length.toString() },
            { id: 'history' as ActiveTab, icon: History, label: 'Run History', badge: runs.length.toString(), badgeColor: 'bg-emerald-500/10 text-emerald-400' },
          ].map(({ id, icon: Icon, label, badge, badgeColor }) => (
            <div
              key={id}
              onClick={() => setActiveTab(id)}
              className={`flex items-center gap-3 px-4 py-3 rounded-2xl cursor-pointer mb-1 transition-colors ${activeTab === id ? 'bg-white/5 text-white' : 'hover:bg-white/5 text-zinc-400'}`}
            >
              <Icon className="w-5 h-5" />
              <span className="font-medium">{label}</span>
              {badge && (
                <div className={`ml-auto text-[10px] px-2 py-px rounded-full ${badgeColor ?? 'bg-zinc-800 text-zinc-400'}`}>{badge}</div>
              )}
            </div>
          ))}
          ...
          {/* Build */}
          <div className="text-[9px] uppercase tracking-[0.12em] text-zinc-600 px-4 mb-2">Build</div>
          {[
            { id: 'flows' as ActiveTab, icon: GitBranch, label: 'Agent Flows', badge: flows.length > 0 ? flows.length.toString() : undefined },
            { id: 'assistants' as ActiveTab, icon: MessageSquare, label: 'Assistants', badge: assistants.length > 0 ? assistants.length.toString() : undefined },
            { id: 'integrations' as ActiveTab, icon: Plug, label: 'Integrations', badge: connectedCount > 0 ? `${connectedCount} on` : undefined, badgeColor: 'bg-blue-500/10 text-blue-400' },
          ].map(({ id, icon: Icon, label, badge, badgeColor }) => (
            <div
              key={id}
              onClick={() => setActiveTab(id)}
        ...
        <div className="p-6 border-t border-zinc-800">
          <div className="glass rounded-3xl p-5 text-xs">
            ...
            <div className="flex items-center justify-between text-[10px]">
              ...
              <div>{connectedCount} integrations</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="h-14 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-xl flex items-center px-8 justify-between z-10">
          <div className="flex items-center gap-2 text-sm">
            <div className="px-4 py-1.5 bg-white/5 rounded-3xl text-white flex items-center gap-2">
              ...
              {activeTab === 'missions' || activeTab === 'fleet' || activeTab === 'history' ? 'Production Fleet' :
               activeTab === 'flows' ? 'Agent Flows' :
               activeTab === 'assistants' ? 'Custom Assistants' : 'Integrations'}
            </div>
          </div>

          <div className="flex items-center gap-6 text-sm">
            {(activeTab === 'missions' || activeTab === 'fleet' || activeTab === 'history') && (
              <button
            ...
            <button onClick={() => setShowSettings(true)} className="text-zinc-400 hover:text-white cursor-pointer transition-colors">
        ...
        {/* Content Area */}
        {isFullPanel ? (
          /* Full-panel views (flows, assistants, integrations) */
          ...
          /* Standard fleet/history/missions layout */
          <div className="flex-1 flex overflow-hidden">
            {/* Fleet / History Sidebar */}
                ...
                {activeTab === 'fleet' && (
                  <div className="p-6">
                    ...
                    <div className="space-y-4">
                      {agents.map((agent) => (
                        <AgentCard key={agent.id} agent={agent} onClick={() => openAgentEditor(agent)} />
                      ))}
                    </div>
                  </div>
                )}
                        ...
                        runs.map((run) => (
                          ...
                          >
                            ...
                            <div className="flex justify-between items-center text-xs">
                              <div className="text-zinc-500">{run.createdAt.toLocaleDateString()}</div>
                    ...
                    <div className="mt-8">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {exampleGoals.map((example, index) => (
                          <div
                            key={index}
                            onClick={() => { addRun(example); setActiveTab('missions'); }}
            ...
            <div className="w-80 border-l border-zinc-800 bg-zinc-900 p-6 hidden lg:flex flex-col">
                  ...
                  <div key={agent.id} className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2.5 h-1 bg-zinc-800 rounded">
                        <div className="h-1 bg-white rounded transition-all" style={{ width: agent.status === 'active' ? '85%' : '35%' }} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {connectedCount > 0 && (
                <div className="mt-8 pt-6 border-t border-zinc-800">
                  <div className="text-[10px] uppercase tracking-[0.5px] text-zinc-500 mb-3">CONNECTED INTEGRATIONS</div>
                  <div className="space-y-2">
                    {Object.values(connections).filter((c) => c.connected).map((c) => (
                      <div key={c.serviceId} className="flex items-center gap-2 text-xs text-zinc-400">
                        <div className="w-2 h-2 rounded-full bg-blue-400" />
                        <span className="capitalize">{c.serviceId.replace('_', ' ')}</span>
                        <span className="ml-auto text-zinc-600 text-[10px] truncate">{c.accountEmail}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-auto pt-8">
                <div className="text-xs text-zinc-400 leading-relaxed border-l-2 border-zinc-700 pl-4">
                  ...
                  Connect Google and Microsoft integrations to give your agents real-world capabilities.
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-2xl w-full mx-4 glass border border-white/10 rounded-3xl overflow-hidden">
            <div className="p-10">
              ...
              <textarea
                ...
                placeholder="Perform a complete competitive teardown of our top 5 competitors and propose a differentiated product strategy..."
              />
            </div>
            <div className="bg-zinc-950 border-t border-zinc-800 p-5 flex justify-end gap-4">
              <button onClick={() => { setShowNewMission(false); setGoalInput(''); }} className="px-8 py-3.5 text-sm">
                CANCEL
              </button>
              <button
                onClick={handleLaunchMission}
      ...
      {/* Agent Editor Modal */}
      <AgentEditor isOpen={showAgentEditor} agent={editingAgent} onClose={closeAgentEditor} />
